...
<script>
	 ...
	 * @example <u-uview-anchor :text="indexList[uview]"></u-uview-anchor>
	 */
	export default {
		name: 'u-index-anchor',
		...
		methods: {
			init() {
				...
				if (!indexList) { 
					return uni.$u.error('u-uview-anchor必须要搭配u-uview-list组件使用')
				}
				// 将当前实例放入到u-uview-list中
				indexList.anchors.push(this)
				const indexListItem = uni.$u.$parent.call(this, 'u-index-item')
				// #ifndef APP-NVUE
				// 只有在非nvue下，u-uview-anchor才是嵌套在u-uview-item中的
				if (!indexListItem) {
					return uni.$u.error('u-uview-anchor必须要搭配u-uview-item组件使用')
				}
				// 设置u-uview-item的id为anchor的text标识符，因为非nvue下滚动列表需要依赖scroll-view滚动到元素的特性
