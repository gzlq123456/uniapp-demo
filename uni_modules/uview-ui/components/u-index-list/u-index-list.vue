	...
	<view class="u-index-list">
		...
		>
			<view
				...
				:class="['u-uview-list__indicator--show']"
				:style="{
	...
	export default {
		...
		methods: {
			init() {
				...
				//解决当u-uview-list组件放在tabbar页面时,scroll-view内容较少时，还能滚动
			...
			getIndexListLetterRect() {
				return new Promise(resolve => {
					// 延时一定时间，以获取dom尺寸
					// #ifndef APP-NVUE
					this.$uGetRect('.u-uview-list__letter').then(size => {
						resolve(size)
					})
					// #endif
			...
			setValueForTouch(currentIndex) {
				...
				// 在非nvue中，由于anchor和item都在u-uview-item中，所以需要对index-item进行偏移
